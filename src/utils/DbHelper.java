...
import data.deduplication.DataDeduplication;
import java.io.File;
...
public class DbHelper {
    ...
    public String userName = DataDeduplication.userName;
    ...
    public Object[][] getAllFilesTableData(){
        ...
        try
        {
            ...
            if(userName.equals("admin")){
                     sql="select distinct file_original_name, file_directory, file_extension, userName from chunks_information";
            }else{
                     sql="select distinct file_original_name, file_directory, file_extension, userName from chunks_information where userName = '"+userName+"'";
            }
            s=con.prepareStatement(sql);
            rs=s.executeQuery();
            
            tableData = new Object[getRecordCount(rs)][4];
            int i = 0;
            while(rs.next())
            {
                tableData[i][0] = i+1;
                ...
                tableData[i][3] = rs.getString("userName");   
                i++;
            }
